...
const DailyLearning = ({ learningData, onUpdate, dayNumber }) => {
    ...
    const LearningItem = ({ icon: Icon, title, content, id, color }) => {
        ...
        return (
            <div className={`group relative p-6 rounded-[2rem] border-2 transition-all duration-500 bg-white ${colors.hoverShadow} ${learningData[id] ? `${colors.border} ${colors.bgLight}` : 'border-slate-50'}`}>
                <header className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        <div className={`p-3 rounded-2xl bg-white shadow-md ${colors.text}`}>
                            <Icon size={18} />
                        </div>
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-600 transition-colors">
                            {title}
                        </h4>
                    </div>
                ...
                <div className={`relative px-1 ${learningData[id] ? 'opacity-60' : 'opacity-100'} transition-opacity`}>
                    <p className="text-slate-900 font-bold leading-relaxed text-sm lg:text-base">
                        {content || '...'}
                    </p>
                </div>
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                ...
                <div className="flex -space-x-3">
                    {[BookOpen, Scroll, Moon, Sun, Heart].map((Icon, i) => (
                        <div key={i} className="w-10 h-10 rounded-full bg-white border-2 border-slate-50 flex items-center justify-center text-slate-400 shadow-sm z-[5-i]">
                            <Icon size={14} />
                        </div>
                    ))}
                </div>
